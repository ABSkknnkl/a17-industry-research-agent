...
- **验收**："主要企业"→ `[隆基绿能， 晶科能源， 天合光能]`（来源 known_entities），查询构造为具体公司。
...
- **改动**
  ...
  - 4 个点位接线：decomposition（拆解出口）、route_decision（路由决策，新增 `layer` 字段区分 `deterministic`/`semantic`）、skill_call（executor 收口）、clarification（澄清门 / 数据缺口门 / unsupported-metrics 回流）；
...
| P0-3 泛称解析 | "主要企业"展开 ≥3 个具体公司   | `隆基绿能 / 晶科能源 / 天合光能`（source=known_entities）                                                                                                | ✅     |
