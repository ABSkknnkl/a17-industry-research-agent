        ...
        if segments:
            ...
            if gen_img:
                ...
                charts.append(ChartSpec(
                    ...
                    option={"series": [{"type": "graph", "data": gen_img.chain_graph.get("nodes") or [], "links": gen_img.chain_graph.get("links") or []}]},
                    evidence_ids=(request.report.evidence_index or {}) and list(request.report.evidence_index.keys())[:1] or ["CHAIN"],
...
3. 【数据适配度与图表选型原则（按数据特征自然选型，严禁为追求形式而生搬硬套）】：
   - 可用图表类型**仅限六种**：line（折线）/ bar（柱状）/ combo（组合）/ area（面积）/ pie（环形占比）/ radar（雷达）。
     除此之外的图型（scatter / bubble / heatmap / boxplot / treemap / industry_chain）一律不得选用。
   - 数据适配优先：必须基于数据维度、量纲与样本量选择最清晰直观的表达形态。
     ...
     - 时间序列分析：采用折线图（line）；
     - 构成 / 占比（各部分份额合计有意义，且各主体均为正数）：采用环形占比图（pie）；
     - 需强调累计规模或份额随时间的堆积变化：采用面积图（area）。
   - 同一指标的 line 与 area 不得重复生成，二者择一：趋势演变用 line，累积/堆积用 area。
    ...
    def _deterministic_fallback(
        ...
        for metric, rows in by_metric.items():
            ...
            if len(comparable) >= 2 and len({x.entity for x in comparable if x.entity}) >= 2:
                ...
                # 构成类指标才适合饼图。原条件过严，会导致 pie 永远无法自动产出：
                #   1) 关键词只覆盖「占比/份额/构成/share」，「市占率」「行业结构」等同义
                #      表述会漏掉；
                #   2) len(vals) <= 5 会把 6~10 项的构成数据整体排除。
                # 放宽为：上界 10 项（再多扇形会碎、不可读）+ 扩充同义词。
                is_composition_metric = any(
                    k in metric.lower()
                    for k in ("占比", "份额", "构成", "市占", "结构", "share", "composition")
                )
                if all(v > 0 for v in vals) and len(vals) <= 10 and is_composition_metric:
