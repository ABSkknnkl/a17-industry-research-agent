...
class DimensionGuard:
    ...
    @staticmethod
    def normalize_financial_value(value: float, metric_name: str, raw_unit: str | None = None) -> tuple[float, str]:
        """Normalizes raw monetary values to legible units (e.g. 亿元).

        E.g. 349,079,082,852.0 (元) -> 3490.79 (亿元)
        """
        meta = resolve_metric_meta(metric_name)

        if meta.dimension == MetricDimension.CURRENCY_AMOUNT:
            # If unit is already 亿元 or 亿
            if raw_unit in ("亿元", "亿"):
                return round(value, 2), "亿元"
            if raw_unit in ("万元", "万"):
                if abs(value) >= 10000:
                    return round(value / 10000, 2), "亿元"
                return round(value, 2), "万元"

            # Raw Yuan or unspecified
            if abs(value) >= 1e8:
        ...
        if meta.dimension == MetricDimension.PERCENTAGE_RATIO:
            # If value is expressed as decimal 0.1544, convert to 15.44%
            # Only if unit is '%' and abs(value) <= 1.0 and non-zero
            if raw_unit in ("%", "百分比") or meta.preferred_unit == "%":
                if 0 < abs(value) <= 1.0:
