...
def select_chain_template_from_text(
    *,
    metric_name: str = "",
    industry_topic: str = "",
    focus_questions: list | None = None,
    core_product_name: str | None = None,
    chain_template_hint: str | None = None,
) -> ChainTemplate:
    if chain_template_hint in ("product_decomposition", "horizontal_flow"):
        return chain_template_hint  # type: ignore[return-value]
    if core_product_name:
        return "product_decomposition"
    focus = focus_questions or []
    focus_text = " ".join(str(item) for item in focus) if isinstance(focus, list) else str(focus)
    text = " ".join([metric_name, str(industry_topic or ""), focus_text])
...
def _clean_node_label(raw: str, max_len: int = 16) -> str:
    """清洗节点名：去掉解读句、指数名、数字、引号碎片，只留短中文产品/环节标签。"""
    s = str(raw or "").strip()
    for ch in "[]'\"\u201c\u201d\u2018\u2019\u3001\uff0c,.\u3002.;\uff1b;\uff1a:()（）":
        s = s.strip(ch)
    s = s.strip()
    if not s:
        return ""
    # 纯数字 / 科学计数
    core = s.replace(".", "").replace("-", "").replace("e", "").replace("E", "")
    if core.isdigit() or s.replace(".", "").replace("-", "").replace("e", "").replace("E", "").isdigit():
        return ""
    try:
        float(s)
        return ""
        ...
        pass
    # 指数/榜单名
    if any(x in s for x in ("漂亮", "出海", "指数", "成份", "成分股", "样本股")):
        return ""
    # 解读句式 / 噪声
    bad = (
        "处于", "拥有", "技术：", "技术:", "占比", "领先", "储备", "趋势", "目前", "为主",
        "领域", "地位", "不足", "是趋势", "半固态", "全固态是", "2.3", "12.", "19.",
    )
    if any(b in s for b in bad):
        return ""
    # 阶段长名压缩
    stage_short = {
        "上游：基础支撑、核心部件与关键供给": "上游供给",
        "中游：核心产品研发、系统集成与方案交付": "中游制造",
        "下游：场景应用、终端落地与商业化生态": "下游应用",
    }
    if s in stage_short:
        return stage_short[s]
    if s.startswith("上游"):
        return "上游供给"
    if s.startswith("中游"):
        return "中游制造"
...
def build_chain_graph_from_segments(
    *,
    title: str,
    subject: str,
    segments: list[dict],
    ...
    """从 data-analysis 的 industry_chain_segments 构建证据约束图谱。

    与同花顺 Agent3 的 verified_chain_graph 同构：
    nodes[{node_id,label,stage,companies,evidence_ids}] + edges[{source,target,label,flow_type,evidence_ids}]
    """
    stage_map = {"upstream": "upstream", "midstream": "midstream", "downstream": "downstream",
                 "上游": "upstream", "中游": "midstream", "下游": "downstream", "support": "support"}
    nodes: list[dict[str, Any]] = []
    edges: list[dict[str, Any]] = []
    allowed_companies: list[str] = []
    seg_node_ids: dict[str, list[str]] = {"upstream": [], "midstream": [], "downstream": [], "support": []}

    for si, seg in enumerate(segments or []):
        stage = stage_map.get(str(seg.get("stage") or ""), "midstream")
        stage_title = {"upstream": "上游供给", "midstream": "中游制造", "downstream": "下游应用", "support": "配套支撑"}.get(stage, "产业环节")
        seg_name = _clean_node_label(seg.get("segment_name") or stage_title) or stage_title
        companies = [str(c).strip() for c in (seg.get("representative_companies") or []) if str(c).strip()]
        allowed_companies.extend(companies)
        evids = [str(e) for e in (seg.get("evidence_record_ids") or seg.get("evidence_ids") or [])] or [f"SEG-{si}"]
        ...
        nodes.append({
            ...
            "logo_names": companies[:3],
            "is_core": False,
            "evidence_ids": evids[:8],
        })
        seg_node_ids[stage].append(nid)

        # 关键产品节点（严格清洗）
        seen = set()
        pi = 0
        for raw in (seg.get("key_products") or []):
            label = _clean_node_label(raw)
            if not label or label in seen or label == seg_name:
                continue
            seen.add(label)
            pi += 1
            pid = f"{nid}P{pi}"
            nodes.append({
                ...
                "logo_names": [],
                "is_core": False,
                "evidence_ids": evids[:4],
            })
            seg_node_ids[stage].append(pid)
            edges.append({
                "source": nid, "target": pid,
                "label": "涵盖", "flow_type": "value", "evidence_ids": evids[:2],
            })

    # 阶段主链：上游→中游→下游
    order = ["upstream", "midstream", "downstream", "support"]
    main_ids = []
    for st in order:
        if seg_node_ids[st]:
            main_ids.append(seg_node_ids[st][0])  # 阶段主节点
    for a, b in zip(main_ids, main_ids[1:]):
        edges.append({
            "source": a, "target": b,
            "label": "供给交付", "flow_type": "supply", "evidence_ids": ["CHAIN-LINK"],
        })

    if not nodes:
        nodes.append({
            "node_id": "S1", "label": subject or title, "stage": "midstream",
            "group": "midstream", "node_kind": "product",
            "companies": [], "logo_names": [], "is_core": True,
            "evidence_ids": ["FALLBACK"],
        })

    tpl = template or select_chain_template_from_text(
        metric_name=title, industry_topic=subject, core_product_name=None,
    )
    return {
        "title": title,
        ...
        "nodes": nodes,
        "edges": edges,
        "evidence_ids": sorted({e for n in nodes for e in n["evidence_ids"]}),
        "allowed_company_names": list(dict.fromkeys(allowed_companies)),
        "allowed_logo_names": list(dict.fromkeys(allowed_companies))[:8],
    }
