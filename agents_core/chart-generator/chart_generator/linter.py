...
BAR_TYPES = {"bar", "comparison_bar", "horizontal_bar", "diverging_bar"}
...
class ChartSkillLinter:
    ...
    def lint(
        ...
        # 1b. Point-Level Grounding Linter
        # 图级 evidence_ids 只能保证“这张图有据可依”，无法保证“图上的每个点都有据”。
        # 逐点校验 point_evidence_ids，防止出现“图有据、点无据”的伪溯源。
        for cand in candidates:
            title = getattr(cand, "title", "未命名图表")
            point_eids = getattr(cand, "point_evidence_ids", None) or []
            if not point_eids:
                continue
            invalid_points = [
                eid for eid in point_eids if eid and str(eid) not in evidence_index
            ]
            if invalid_points:
                violations.append(ChartLinterViolation(
                    code="hallucinated_point_evidence",
                    message=(
                        f"图表《{title}》有 {len(invalid_points)} 个数据点引用了不存在的证据ID: "
                        f"{invalid_points[:5]}"
                    ),
                    chart_title=title,
                    severity="error",
        ...
        for cand in candidates:
            ...
            # 3.2 Treemap requires sufficient categories (>= 8 items) and additive metric
            if ctype == "treemap":
                ...
                for eid in eids:
                    if eid in evidence_index:
                        ...
                        if hasattr(ref, "metric") and ref.metric:
                            cand_metrics.append(str(ref.metric).lower())
                        elif isinstance(ref, dict) and ref.get("metric"):
                            cand_metrics.append(str(ref["metric"]).lower())
                is_non_additive = any(k in title_lower for k in non_additive_keywords) or any(
                    any(k in m for k in non_additive_keywords) for m in cand_metrics
                )
                if is_non_additive:
                    violations.append(ChartLinterViolation(
                        code="treemap_non_additive_metric",
                        message=(
                            f"图表《{title}》使用矩形树图展示不可加总的强度量（如价格/比率/估值倍数）。"
                            f"矩形树图面积严格表示部分占整体的比例，只允许用于加总有意义的广延量（如市值、营收、资产规模），"
                            f"严禁用于股价或比率"
                        ),
                        chart_title=title,
                        severity="error",
            ...
            # 3.3 Radar requires sufficient dimensions (>= 3 indicators)
            if ctype == "radar":
                indicators = []
                if isinstance(option, dict):
                    r_cfg = option.get("radar", {})
                ...
                if 1 <= len(indicators) < 3:
                    violations.append(ChartLinterViolation(
                        code="radar_insufficient_dimensions",
                        message=(
                            f"图表《{title}》使用雷达图但仅有 {len(indicators)} 个评估维度 (<3)，几何图形退化为线段。"
                            f"雷达图必须具备至少 3 个互补评价维度，少于 3 个应改用条形图或双轴组合图"
                        ),
                        chart_title=title,
                        severity="error",
                    ))
        ...
        # 3.5 Redundant macro metric in standalone line/area when combo already covers it
        macro_combo_metrics: set[str] = set()
        for cand in candidates:
            if getattr(cand, "chart_type", "") == "combo":
        ...
        if macro_combo_metrics:
            for cand in candidates:
                if getattr(cand, "chart_type", "") in ("line", "area"):
                    cand_m = set()
                    for eid in getattr(cand, "evidence_ids", []):
                        if eid in evidence_index:
                            ref = evidence_index[eid]
                    ...
                    if cand_m and cand_m.issubset(macro_combo_metrics):
                        violations.append(ChartLinterViolation(
                            code="redundant_macro_metric",
                            message=f"图表《{getattr(cand, 'title', '')}》展示的宏观指标已在综合组合图中完整呈现，单独生成趋势图属于重复冗余",
                            chart_title=getattr(cand, "title", ""),
                            severity="error",
                        ))
                        break
        ...
        for cand in candidates:
            ...
            ctype = getattr(cand, "chart_type", "")
            eids = getattr(cand, "evidence_ids", [])
            if ctype not in ("combo", "dual_axis", "dual_axis_combo", "radar", "scatter", "bubble", "industry_chain", "heatmap") and len(eids) >= 2:
