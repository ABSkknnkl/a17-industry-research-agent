...
logger = logging.getLogger("chart_compiler")
...
class EChartsCompiler:
    ...
    def compile(
        ...
        Args:
            table: 规范化数据表。
            chart_type: 六图之一（或向后兼容的历史别名）。
            title: 图标题。
            variant: 复合图的呈现变体，目前支持 combo 的 "dual_axis" / "dual_panel"。
                variant 只描述六图**内部**的呈现差异，不改变 chart_type 语义，
                也不得用于绕开门禁。
        """
        ...
        # 1. Combo（双轴 / 双面板两种变体）
        if t_str in ("combo", "dual_axis_combo", "dual_axis", "dual_panel"):
            if variant == "dual_panel" or t_str == "dual_panel":
                return EChartsCompiler._compile_dual_panel(table, title, base_grid)
        ...
        # 下排图型（scatter / bubble / heatmap / boxplot / treemap / industry_chain）
        # 已从产品中禁用。此处不再保留 scatter/bubble 分支，使这些图型无法经由
        # 本编译器产出 —— 门禁在 agent 层以 normalize_active_chart_type 过滤，
        # 这里再断掉一条潜在通路。

        # 兜底：仅用于「已是六图之一、但数据形态与该图型不匹配」的降级场景，
        # 不改变图型语义。加日志而非静默，便于观测异常选型。
        fallback = "line" if table.axis_type == AxisType.TIME_SERIES else "bar"
        logger.warning(
            "图表类型 %r 与数据形态 %s 不匹配，降级为 %s",
            t_str,
            getattr(table.axis_type, "value", table.axis_type),
            fallback,
        )
        if fallback == "line":
    ...
    @staticmethod
    def _compile_dual_panel(
        table: NormalizedDataTable,
        title: str,
        grid: dict[str, Any],
    ) -> dict[str, Any]:
        """左右双面板复合图：两个并列坐标系，各自独立的 X / Y 轴。

        与 dual_axis 的区别：
        - dual_axis —— **同一**坐标系内挂两个 Y 轴，适用于量纲可比（如"亿元"与"%"）
          但数值区间差异大的场景，两轴共享横轴与绘图区；
        - dual_panel —— **两个**并列坐标系，各自横轴，适用于两组指标量纲完全不同
          （如"营收金额"与"资产负债率"）、放在同一绘图区会互相干扰的场景。

        配色复用本项目出版级 PALETTE（主蓝 / 琥珀），不引入额外色板。

        Args:
            table: 规范化数据表，取前两个系列分别放入左右面板。
            title: 图标题。
            grid: 基础网格参数（此处仅作参考，双面板需各自的布局）。

        Returns:
            ECharts option；系列不足 2 个时退化为普通柱状图。
        """
        names = list(table.series_data.keys())
        if len(names) < 2:
            return EChartsCompiler._compile_bar(table, title, grid)

        left_name, right_name = names[0], names[1]
        left_data = table.series_data.get(left_name, [])
        right_data = table.series_data.get(right_name, [])
        left_unit = table.series_units.get(left_name, "")
        right_unit = table.series_units.get(right_name, "")

        def _axis(unit_text: str) -> dict[str, Any]:
            return {
                "type": "value",
                "name": unit_text,
                "splitNumber": 4,
        ...
        def _x_axis(grid_index: int) -> dict[str, Any]:
            return {
                "type": "category",
        ...
        def _panel_series(
            name: str, data: list[float], grid_index: int, color: str
        ) -> dict[str, Any]:
            return {
                "name": name,
                "type": "bar",
        ...
        return {
            ...
            "legend": {"data": [left_name, right_name], "top": 4},
            # 两个并列 grid：左面板占左半，右面板占右半，中间留白区隔
            "grid": [
                {"left": 56, "right": "53%", "top": 52, "bottom": 40, "containLabel": True},
                {"left": "57%", "right": 40, "top": 52, "bottom": 40, "containLabel": True},
            ],
            "xAxis": [_x_axis(0), _x_axis(1)],
            "yAxis": [_axis(left_unit), _axis(right_unit)],
            "series": [
                _panel_series(left_name, left_data, 0, PRIMARY_COLOR),
                _panel_series(right_name, right_data, 1, ACCENT_COLOR),
            ],
        }
