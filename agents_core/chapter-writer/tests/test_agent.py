...
def test_without_llm_returns_complete_fallback():
    result=asyncio.run(ChapterWriterAgent(llm=MissingLLM()).run(ChapterWritingRequest(report=report()),save_artifacts=False))
...
def test_fast_skill_router_policy_routing_and_linter_events():
    ...
    agent = ChapterWriterAgent(llm=MissingLLM())  # LLM unavailable -> uses FastSkillRouter
