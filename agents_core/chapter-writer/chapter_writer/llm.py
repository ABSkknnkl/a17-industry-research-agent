    ...
    async def plan_skills_with_tools(
        ...
        for attempt in range(3):
            ...
            try:
                ...
                if attempt < 2:
                    logger.warning("plan_skills attempt %d failed (%s), retrying...", attempt + 1, err_msg)
    ...
    async def generate_json(self, system: str, user: str) -> dict[str, Any]:
        ...
        for attempt in range(3):
            ...
            try:
                ...
                err_msg = str(e) or type(e).__name__
                if attempt < 2:
                    logger.warning("chapter generate_json attempt %d failed (%s), retrying...", attempt + 1, err_msg)
                    if isinstance(e, (httpx.TimeoutException, httpx.HTTPError)):
                        try:
                            await client.aclose()
                        except Exception:
