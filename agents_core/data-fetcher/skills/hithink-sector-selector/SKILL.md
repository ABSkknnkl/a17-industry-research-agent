...
| `X-Claw-Skill-Id` | `hithink-sector-selector`（与 skill name 一致） |
...
**注意：** 默认返回 10 条数据，但符合条件的板块总数可能更多，需关注 `code_count` 字段并通过分页获取全部数据。
...
```python
for item in datas:
    sector_name = item.get("板块名称")       # 如 "半导体"
...
| code_count | INT | 符合查询条件的总板块数量（注意：可能大于当前返回的 datas 条数） |
...
python3 scripts/cli.py --query "今日涨幅最大的板块有哪些？" --call-type "retry"

# 指定超时时间（复杂查询可适当增加）
python3 scripts/cli.py --query "今日涨幅最大的板块有哪些？" --timeout 60
```
...
} -Body '{"query":"涨幅前五的板块","page":"1","limit":"10","is_cache":"1","expand_index":"true"}'
```
