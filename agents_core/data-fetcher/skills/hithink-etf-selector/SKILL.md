---
name: hithink-etf-selector
description: 根据行情、跟踪指数基本面、规模、风格类型等条件筛选ETF。返回符合条件的相关ETF数据。当用户询问ETF筛选问题时，必须使用此技能。
...
| `X-Claw-Skill-Id` | `hithink-etf-selector`（与 skill name 一致） |
...
headers = {
    ...
    "X-Claw-Skill-Id": "hithink-etf-selector",
...
**注意：** 默认返回 10 条数据，但符合条件的ETF总数可能更多，需关注 `code_count` 字段并通过分页获取全部数据。
...
```python
for item in datas:
    etf_code = item.get("ETF代码")       # 如 "510300.SH"
    etf_name = item.get("ETF简称")       # 如 "华泰柏瑞沪深300ETF"
    # 其他字段根据查询类型而定
```
...
| code_count | INT | 符合查询条件的总ETF数量（注意：可能大于当前返回的 datas 条数） |
...
python3 scripts/cli.py --query "沪深300ETF有哪些？" --call-type "retry"

# 指定超时时间（复杂查询可适当增加）
python3 scripts/cli.py --query "沪深300ETF有哪些？" --timeout 60
```
...
curl -X POST "https://openapi.iwencai.com/v1/query2data" \
  ...
  -H "X-Claw-Skill-Id: hithink-etf-selector" \
...
} -Body '{"query":"沪深300ETF","page":"1","limit":"10","is_cache":"1","expand_index":"true"}'
```
