---
name: hithink-futures-query
description: 查询期货期权的行情、波动率、产销、会员持仓、会员榜单、行权等多种数据，支持自然语言问句输入，返回相关期货期权数据结果。当用户询问期货期权的行情、波动率、产销、会员持仓、会员榜单、行权等问题时，必须使用此技能。
...
接收用户的自然语言查询请求，分析用户意图，识别期货期权数据相关查询类型：
- 期货期权行情查询
- 波动率/产销数据查询
- 会员持仓/榜单查询
- 行权数据查询
...
| `X-Claw-Skill-Id` | `hithink-futures-query`（与 skill name 一致） |
...
```python
for item in datas:
    futures_code = item.get("期货代码")       # 如 "CU2505.SHF"
    futures_name = item.get("期货简称")       # 如 "沪铜2505"
    # 其他字段根据查询类型而定，如最新价、涨跌幅、持仓量、波动率等
```
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| datas | ARRAY | 金融数据列表，对象数组，每个对象包含期货代码、期货简称、最新价、涨跌幅、持仓量等字段 |
...
{
  "datas": [
    {"期货代码": "CU2505.SHF", "期货简称": "沪铜2505", "最新价": "78520", "涨跌幅": "1.25%", "持仓量": "125680"}
  ],
  "code_count": 256,
  "chunks_info": {
    "query": "沪铜期货最新行情",
    "parsed_conditions": ["沪铜期货", "最新行情"]
  }
}
```
...
# 指定超时时间（复杂查询可适当增加）
python3 scripts/cli.py --query "沪铜期货最新行情" --timeout 60
```
...
curl -X POST "https://openapi.iwencai.com/v1/query2data" \
  ...
  -H "X-Claw-Skill-Id: hithink-futures-query" \
...
} -Body '{"query":"沪铜期货最新行情","page":"1","limit":"10","is_cache":"1","expand_index":"true"}'
```
