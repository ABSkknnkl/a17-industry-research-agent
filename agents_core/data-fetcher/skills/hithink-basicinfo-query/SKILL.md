---
name: hithink-basicinfo-query
description: 查询全品类标的（股票、指数、基金、期货、期权、转债、债券、理财、保险等）的基础信息、发行主体、机构资料、费率、上市地点、上市日期等静态信息，支持自然语言问句输入，返回相关基本资料数据结果。当用户询问股票基本信息、基金资料、期货合约信息、债券资料、费率信息、上市日期等基本资料查询问题时，必须使用此技能。
...
接收用户的自然语言查询请求，分析用户意图，识别基本资料相关查询类型：
- 股票基本信息查询
- 基金/债券/期货基本信息查询
- 上市日期/费率/评级查询
...
|-------------|-----------|
| 同花顺什么时候上市的 | 同花顺上市日期 |
| 贵州茅台的基本信息 | 贵州茅台股票代码 股票简称 所属行业 上市日期 |
| 华夏成长混合的费率多少 | 华夏成长混合基金费率 |
| 沪铜2505合约详情 | 沪铜2505期货合约代码 到期日 交割方式 |
...
| `X-Claw-Skill-Id` | `hithink-basicinfo-query`（与 skill name 一致） |
...
{
  "datas": [
    {"股票代码": "300033.SZ", "股票简称": "同花顺", "上市日期": "20091225"}
  ],
  "code_count": 5236,
  "chunks_info": {
    "query": "同花顺上市日期",
    "parsed_conditions": ["同花顺", "上市日期"]
  }
}
```
...
# 查询基金费率
python3 scripts/cli.py --query "华夏成长混合基金费率"

# 查询期货合约信息
python3 scripts/cli.py --query "沪铜2505期货合约详情"

# 翻页查询
python3 scripts/cli.py --query "股票基本信息" --page 2 --limit 20

# 重试请求（放宽条件后使用 retry 标记）
python3 scripts/cli.py --query "同花顺上市日期" --call-type "retry"

# 指定超时时间（复杂查询可适当增加）
python3 scripts/cli.py --query "同花顺上市日期" --timeout 60
```
...
} -Body '{"query":"同花顺上市日期","page":"1","limit":"10","is_cache":"1","expand_index":"true"}'
```
