...
class ReportFusionAgent:
    ...
    def _default_summary(self,request:ReportFusionRequest)->ExecutiveSummary:
        conclusions=[]
        for item in request.report.insights[:6]:
        ...
        if not conclusions:
            for chapter in request.chapters.chapters[:4]:conclusions.append(ReportConclusion(text=chapter.summary,evidence_ids=chapter.evidence_ids[:8],uncertainty="需结合章节中的资料边界理解"))

        metric_cards: list[MetricCardDraft] = []
        for sec in [s for ch in request.chapters.chapters for s in ch.sections]:
            if sec.metric_cards:
                metric_cards.extend(sec.metric_cards)
                if len(metric_cards) >= 4:
                    break
        if not metric_cards:
            for km in getattr(request.report, "key_metrics", [])[:4]:
                if isinstance(km, dict):
                    metric_cards.append(MetricCardDraft(
                        label=str(km.get("name") or km.get("metric") or "核心指标"),
                        value=str(km.get("value", "-")),
                        unit=km.get("unit"),
                        period=str(km.get("period") or ""),
                        evidence_ids=km.get("evidence_record_ids", [])
                    ))
    ...
    def _apply_editorial(self,raw:dict[str,Any],request:ReportFusionRequest,chapters:list[ChapterDraft])->tuple[ExecutiveSummary,list[ChapterDraft],dict[str,str],int,int,list[str]]:
        ...
        risks = [_extract_str(x) for x in raw.get("risks", []) if _extract_str(x) and valid_text(_extract_str(x))] or default.risks
        boundaries = [_extract_str(x) for x in raw.get("research_boundaries", []) if _extract_str(x) and valid_text(_extract_str(x))] or default.research_boundaries
    ...
    def _embed_charts(self,request:ReportFusionRequest,chapters:list[ChapterDraft],warnings:list[str])->list[EmbeddedChart]:
        ...
        placements={cid:s.section_id for ch in chapters for s in ch.sections for cid in s.chart_ids}
        result=[]
        for cid,chart in chart_map.items():
            ...
            placement_id = placements.get(cid)
            if not placement_id:
                rec_ch = getattr(chart, "recommended_chapter_id", None)
                for ch in chapters:
                    if rec_ch and ch.chapter_id == rec_ch and ch.sections:
                        placement_id = ch.sections[0].section_id
                        break
                    elif "现金" in chart.title and "05" in ch.chapter_id:
                        for s in ch.sections:
                            if "现金" in s.title:
                                placement_id = s.section_id
                                break
                        ...
                        for s in ch.sections:
                            if "收入" in s.title or "规模" in s.title or "盈利" in s.title:
                                placement_id = s.section_id
                                break
                if not placement_id and chapters and chapters[0].sections:
                    placement_id = chapters[0].sections[0].section_id
