...
### 1. 安装 Python 后端及智能体依赖
```bash
pip install -r requirements.txt
# （可选）若需生成高保真 PDF 矢量研报，执行：playwright install chromium
```
...
### 方式一：前端界面可视化配置（推荐，零门槛）
本系统内置了现代化的**前端配置面板与实时连通性探测器**，**无需手动修改任何配置文件**：
1. 启动前后端服务（参见下方启动说明）；
2. 浏览器访问 `http://localhost:5173`；
3. 点击页面右上角的 **「模型与接口配置」** 按钮；
4. **一键切换服务商预设**（火山引擎方舟、DeepSeek 官方、硅基流动、本地 Ollama 等）；
5. 粘贴您的 `大模型 API Key` 与 `同花顺问财 API Key`；
6. 点击 **「测试模型连通性」** 与 **「测试问财连通性」**，确认绿灯通过后点击 **「保存并应用」**；
7. 配置将立即在后端五智能体引擎中生效并自动持久化，开箱即用。

### 方式二：.env 配置文件配置（适合服务器/无头环境）
若您习惯使用环境配置文件：
```bash
cp .env.example .env
# 编辑 .env 文件，填入您的大模型 API 凭证 (LLM_API_KEY) 与同花顺问财凭证 (IWENCAI_API_KEY)
```
