...
export const skillLabel = (skill: string): string => {
  ...
  return SKILL_LABELS[normalized] ?? SKILL_LABELS[trimmed] ?? DOMAIN_LABELS[normalized] ?? trimmed
}

export const dimensionLabel = (dim?: string): string => (dim ? (DOMAIN_LABELS[dim] ?? dim) : '')

export const intentPlanLabel = (question?: string): string => {
  if (!question) return ''
  const trimmed = question.trim()
  const domain = trimmed.replace(/\s*核心数据$/, '')
  const label = dimensionLabel(domain)
  return label === domain ? trimmed : label
}
