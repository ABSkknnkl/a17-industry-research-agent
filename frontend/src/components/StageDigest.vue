...
import { dimensionLabel, intentPlanLabel, skillLabel } from '../api/labels'
  ...
  <div class="digest">
      ...
      <template v-if="intentPlanEntries.length > 0">
        ...
        <el-table :data="intentPlanEntries" size="small" border>
          <el-table-column label="研究问题" min-width="220">
            <template #default="{ row }">{{ intentPlanLabel(row[0]) }}</template>
          </el-table-column>
      ...
      <template v-else>
        <div class="stage-pending-box">
          ...
          <p class="pending-desc">
            底层智能体正在基于投研需求分解 7
            大维度，并并发调度问财金融技能抓取实体与财务指标。采集完成后将自动在此渲染结构化指标列表与意图路由明细。
          </p>
    ...
    <template v-else-if="stage === 'data_interpret'">
      ...
      <template v-else>
        <div class="stage-pending-box">
          ...
          <p class="pending-desc">
            正在基于阶段一全量数据集执行确定性复合增速 CAGR 测算、稳健 Z
            分数离群异常检测、三表勾稽验证与 6 维投研方法论洞察提炼。
          </p>
          <div class="pending-steps">
    ...
    <template v-else-if="stage === 'chart_generate'">
      <template v-if="chartSpecs.length > 0">
        ...
        <ChartGallery :specs="chartSpecs" :run-id="runId" />
      </template>
      <template v-else>
        <div class="stage-pending-box">
          <div class="pending-lead">
          ...
          <p class="pending-desc">
            正在分析数据形态规划出版级图表矩阵，结合 ECharts 引擎渲染 960x520
            矢量图表并进行排版自愈与审美校验。
          </p>
          <div class="pending-steps">
    ...
    <template v-else-if="stage === 'chapter_write'">
      <template v-if="chapters.length > 0">
        ...
        >
          ...
          >
            ...
            <p v-for="p in sectionParagraphs(sec)" :key="p.id" class="article-para">
              {{ p.text }}
            </p>
          </section>
        </article>
      ...
      <template v-else>
        <div class="stage-pending-box">
          ...
          <p class="pending-desc">
            7 章 21
            节券商深度专题骨架已激活，正在并发组织写作方法论技能调度，并穿透关联客观证据与矢量图表。
          </p>
          <div class="pending-steps">
    ...
    <template v-else-if="stage === 'report_fusion'">
      <div class="stage-pending-box">
        ...
        <p class="pending-desc">
          正在组织 4 大总编审校技能协同审计，统合数据口径、编纂证据穿透目录并编译导出 Markdown /
          HTML / PDF 多格式报告。
        </p>
        <div class="pending-steps">
