...
import { fieldLabel, skillLabel } from '../api/labels'
...
const totalRecordCount = computed<number>(() => {
  ...
  if (typeof d.total_records === 'number') return d.total_records
  if (typeof d.evidence_count === 'number') return d.evidence_count
  return sourceList.value.length
})
...
const filteredRecords = computed(() => {
  ...
  if (searchKeyword.value.trim()) {
    ...
    list = list.filter((r) => {
      ...
      return ent.includes(q) || code.includes(q) || metric.includes(q) || val.includes(q) || qText.includes(q)
    })
  }
  return list
})
  ...
  <div class="data-fetch-digest" data-testid="data-fetch-digest">
    ...
    <div class="fetch-stats-strip">
      <div class="stat-group">
        <span class="stat-main">
          采集来源明细（<template v-if="totalRecordCount > sourceList.length">精选 {{ sourceList.length }} 条 / 全量 {{ totalRecordCount }} 条</template><template v-else>{{ sourceList.length }} 条</template>）
        </span>
      ...
      <div class="domain-pills">
        <span
          ...
          @click="selectedDomain = 'all'; currentPage = 1"
        >
        ...
        <span
          ...
          @click="selectedDomain = dom; currentPage = 1"
        >
    ...
    <div v-if="annotationStats.total > 0" class="annotation-bar">
      <span class="annot-summary">
        已标注数据样本：
        <span v-if="annotationStats.emphasize > 0" class="annot-tag is-emp">重点关注 {{ annotationStats.emphasize }} 条</span>
        <span v-if="annotationStats.reject > 0" class="annot-tag is-rej">建议剔除 {{ annotationStats.reject }} 条</span>
      </span>
      <span class="annot-hint">（批注将指导阶段二数据解读智能体在做财务建模与回归时做加权或过滤）</span>
    </div>
    ...
    <el-collapse v-if="intentPlanEntries.length > 0" class="intent-collapse">
      <el-collapse-item name="plans">
        <template #title>
          <span class="collapse-title">查看数据获取意图路由计划 ({{ intentPlanEntries.length }} 个领域子任务)</span>
        </template>
          ...
          <div v-for="[key, plan] in intentPlanEntries" :key="key" class="plan-card">
            <div class="plan-head">
              ...
              <el-tag size="small" effect="plain">
                {{ (plan as any).skill_name || plan.sub_requirements?.[0]?.candidate_skills?.[0] || '问财投研' }}
              </el-tag>
            </div>
            <p class="plan-query muted">
              {{ (plan as any).query || (plan.sub_requirements?.[0] as any)?.query || plan.sub_requirements?.[0]?.description || '基于主题与约束自动推演' }}
            </p>
          </div>
        </div>
      </el-collapse-item>
