...
import { dimensionLabel, metricLabel } from '../api/labels'
...
const filteredInsights = computed(() => {
  ...
  if (filterConfidence.value !== 'all') {
    list = list.filter(
      (item) => String(item.confidence || '').toLowerCase() === filterConfidence.value
    )
  }
...
const evidenceDigest = computed<Record<string, Record<string, unknown>>>(() => {
  const raw = props.data.evidence_digest
  return raw && typeof raw === 'object' && !Array.isArray(raw)
    ? (raw as Record<string, Record<string, unknown>>)
    : {}
})

const sourceRecordIndex = computed<Record<string, Record<string, unknown>>>(() => {
  const index: Record<string, Record<string, unknown>> = {}
  for (const record of props.sourceRecords ?? []) {
    const recordId = String(record.record_id || '')
    if (recordId) index[recordId] = record
  }
  return index
})
...
function evidenceLabel(recordId: string): string {
  const evidence = evidenceDigest.value[recordId] || sourceRecordIndex.value[recordId]
  if (!evidence) return recordId

  const entity = String(evidence.entity || evidence.entity_name || '')
  const metric = metricLabel(String(evidence.metric || ''))
  const prefix = [entity, metric].filter(Boolean).join(' · ')
  const value = evidence.value
  const valueText =
    typeof value === 'string' || typeof value === 'number'
      ? ` ${value}${String(evidence.unit || '')}`
      : ''

  return prefix ? `${prefix}${valueText}` : String(evidence.label || recordId)
}

function riskKindLabel(kind: unknown): string {
  const labels: Record<string, string> = {
    cross_sectional_outlier: '同业离群',
    time_series_spike: '时序突变',
    source_conflict: '来源冲突',
    data_quality: '数据缺失',
  }
  return labels[String(kind || '')] || '数据异常'
}

function riskSummary(risk: Record<string, unknown>): string {
  const subject = [risk.entity, risk.metric].filter(Boolean).join(' · ')
  const details: string[] = []

  if (risk.observed_value !== null && risk.observed_value !== undefined) {
    details.push(`观测值 ${formatVal(risk.observed_value)}`)
  }
  if (risk.expected_range) details.push(`参考区间 ${risk.expected_range}`)
  if (risk.period) details.push(`期间 ${risk.period}`)

  if (details.length > 0) return `${subject || risk.title || risk.risk_code}：${details.join('，')}`
  if (risk.kind === 'data_quality')
    return `${subject || risk.title || risk.risk_code}：数据字段不完整，建议复核原始记录`
  return String(subject || risk.title || risk.message || risk.risk_code || '待复核数据点')
}
</script>
...
<template>
  <div class="interpretation-digest" data-testid="interpretation-digest">
      ...
      <div class="hero-header">
        ...
        <div class="hero-meta">
          <span class="stat-pill"
            ><b>{{ insights.length }}</b> 项深度洞察</span
          >
          <span class="stat-pill"
            ><b>{{ knowledgeFacts.length }}</b> 条原子事实</span
          >
          <span class="stat-pill"
            ><b>{{ keyMetrics.length }}</b> 项核心指标</span
          >
        </div>
      </div>
    ...
    <div v-if="annotationStats.total > 0" class="annotation-bar">
      <span class="annot-icon">✍️</span>
      <span class="annot-summary">
        当前已标注 <b>{{ annotationStats.total }}</b> 项对象：
        <span v-if="annotationStats.emphasize > 0" class="annot-tag is-emp"
          >🌟 重点强调 {{ annotationStats.emphasize }} 项</span
        >
        <span v-if="annotationStats.doubt > 0" class="annot-tag is-dbt"
          >❓ 存疑核实 {{ annotationStats.doubt }} 项</span
        >
        <span v-if="annotationStats.reject > 0" class="annot-tag is-rej"
          >🚫 建议剔除 {{ annotationStats.reject }} 项</span
        >
      </span>
      <span class="annot-hint"
        >（这些批注已实时同步至协同工作台，点击「修改条件重跑」即可定向优化）</span
      >
    </div>
    ...
    <div class="digest-tabs-header">
      <el-radio-group v-model="activeTab" size="small" class="digest-nav-radios">
        ...
        <el-radio-button value="coverage">
          🛡️ 维度覆盖与风险 ({{ dimensionCoverage.length }})
        </el-radio-button>
      </el-radio-group>
        ...
        >
          <header class="card-head">
            <div class="head-left">
              ...
              <el-tag
                :type="confidenceTagType(ins.confidence)"
                size="small"
                effect="plain"
                class="conf-pill"
              >
            ...
            <div class="annotation-actions">
              ...
              <el-tooltip content="对结论存疑，要求重新核实" placement="top">
                <button
                  ...
                  @click="
                    toggleAnnotation(String(ins.insight_id), 'insight', String(ins.title), 'doubt')
                  "
                >
                  ❓ 存疑
                </button>
              ...
              <el-tooltip content="剔除该洞察，不写入最终报告" placement="top">
                <button
                  ...
                  @click="
                    toggleAnnotation(String(ins.insight_id), 'insight', String(ins.title), 'reject')
                  "
                >
                  🚫 剔除
                </button>
    ...
    <div v-else-if="activeTab === 'metrics'" class="tab-content metrics-view">
      ...
      <div v-if="keyMetrics.length > 0" class="sub-section">
        ...
        <div class="metrics-grid">
          <div
            v-for="m in keyMetrics.slice(0, 12)"
            :key="String(m.metric_id)"
            class="metric-pill-card"
          >
        ...
        <div v-if="anomalies.length > 0" class="duo-col">
          ...
          <ul class="anomaly-list">
            <li v-for="(a, idx) in anomalies.slice(0, 8)" :key="idx" class="anomaly-item">
              <span class="anomaly-tag">⚠️ 异动</span>
      ...
      <div v-if="crossValidations.length > 0" class="sub-section" style="margin-top: 14px">
        ...
        <div class="cv-grid">
          ...
          >
            <span class="cv-status">{{ cv.passed ? '✓ 验证通过' : '⚠️ 勾稽异常' }}</span>
    ...
    <div v-else-if="activeTab === 'coverage'" class="tab-content coverage-view">
      <div v-if="dimensionCoverage.length > 0" class="sub-section">
        ...
        <div class="cov-pills">
          ...
          >
            <div class="cov-head">
              <strong>{{
                cov.dimension_label || dimensionLabel(String(cov.dimension || ''))
              }}</strong>
              <el-tag
                size="small"
                :type="cov.status === 'covered' ? 'success' : 'warning'"
                effect="plain"
              >
      ...
      <div v-if="risks.length > 0" class="sub-section" style="margin-top: 16px">
        <h5 class="sub-title">数据异常检测（{{ risks.length }} 条）</h5>
        <p class="risk-intro muted">
          系统自动识别偏离同业或历史规律的数据点，建议结合原始记录复核。
        </p>
        <div class="risk-cards">
          <div
            v-for="(r, idx) in risks"
            :key="String(r.risk_code || idx)"
            class="risk-card"
            :class="`severity-${String(r.severity || 'medium')}`"
          >
            <span class="risk-badge">{{ riskKindLabel(r.kind) }}</span>
            <span class="risk-content">{{ riskSummary(r) }}</span>
          </div>
        </div>
      </div>
    </div>
...
.cv-card.is-pass .cv-status {
  color: #10b981;
}
.cv-card.is-warn .cv-status {
  color: #f59e0b;
}
.cv-name {
  font-size: 12px;
