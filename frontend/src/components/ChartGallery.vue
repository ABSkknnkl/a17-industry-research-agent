  ...
  for (const s of seriesList) {
    ...
    if (!hasCoordinates) {
      ...
      for (const [key, conf] of Object.entries(stageMap)) {
        ...
        const stepY = count === 1 ? 0 : Math.min(75, 260 / (count - 1))
        const startY = count === 1 ? 210 : 105 + (260 - (count - 1) * stepY) / 2

        gNodes.forEach((node, idx) => {
          const y = startY + idx * stepY
          const nodeName = String(node.name || `节点${idx + 1}`)
    ...
    if (Array.isArray(s.links)) {
      s.links = (s.links as Record<string, unknown>[]).map((link) => ({
        ...link,
        lineStyle: {
          color: '#98a2b3',
          width: 2,
    ...
    option.tooltip = {
      ...
      formatter: (params: unknown) => {
        ...
        if (p.dataType === 'edge') {
          ...
          return `<div style="color:#475467;">供需流向: ${edge.data?.source ?? ''} → ${edge.data?.target ?? ''}</div>`
        }
        return ''
      },
    }
