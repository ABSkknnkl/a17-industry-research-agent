...
describe('AgentLiveTrace 智能体执行动线组件', () => {
  ...
  it('抽屉模式下正确渲染动线列表过渡组与协同指挥舱', async () => {
    vi.spyOn(client, 'getRunEvents').mockResolvedValue(mockEvents)
    ...
    await vi.waitFor(() => {
      expect(wrapper.text()).toContain('智能体全链路协同指挥')
      expect(wrapper.text()).toContain('微观执行动线时序')
      expect(wrapper.text()).toContain('3 条')
    })

    // 验证动线列表具备 TransitionGroup 与动效类
    const streamList = wrapper.find('.event-stream-list')
    expect(streamList.exists()).toBe(true)
    const eventRows = wrapper.findAll('.event-row')
    expect(eventRows.length).toBe(3)
  })

  it('筛选类型后点击刷新，立即全量输出匹配事件，无延迟与空白卡滞', async () => {
    vi.spyOn(client, 'getRunEvents').mockResolvedValue(mockEvents)

    const wrapper = mount(AgentLiveTrace, {
      props: {
        runId: 'test-run-filter-refresh',
        isRunning: false,
        mode: 'drawer',
      },
      global: { plugins: [ElementPlus, createPinia()] },
    ...
    await vi.waitFor(() => {
      expect(wrapper.findAll('.event-row').length).toBe(3)
    })
    ...
    // 刷新后所有事件立即呈现，不会因为 0.5s 队列变成空状态
    await vi.waitFor(() => {
      const rows = wrapper.findAll('.event-row')
      expect(rows.length).toBe(3)
      expect(wrapper.text()).not.toContain('暂无匹配的智能体执行事件')
    })
  })
})
