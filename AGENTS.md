...
1. **先问再动不可逆操作**：push 远端、删分支、改 `main`、开分支保护，未经用户确认不要做。
2. **拉协作者**：用户已给出 GitHub 用户名时**不要再问一遍**，可直接执行；仅在缺用户名时才询问。
3. **未验证代码进隔离分支**，不要直接怼 `main`。
4. **硬拦截清单之外全部放行**；不要擅自加严（例如拦 AI prompt、拦姓名、拦大文件、强制 PR）。
5. **不要提交**测试产物、密钥、AI 本地记忆目录。
6. 改业务代码前先搜索现有实现；能改就不要新建平行模块。
...
### 4.1 唯一工作分支（2026-09-14 起生效）

```
agent/chart-capability-code     ← 项目负责人（ABSkknnkl）的专属工作分支
```

**所有提交默认写在这条分支上。不要再新建分支。**

| MUST | MUST NOT |
| --- | --- |
| 所有改动提交到 `agent/chart-capability-code` | 因「一任务一分支」「一会话一分支」自建分支 |
| 新分支只在用户明确说「新开分支 / 开个分支做 X」时创建 | 用日期、任务名、工具名自动生成分支名 |
| 新建分支前先向用户报分支名并等确认 | 未经确认就 `git checkout -b` + `git push` |
| 合并进 main 后删除该临时分支 | 合完不删，留下孤儿分支 |

**背景**：2026-09-11 至 09-13 期间，不同 AI 会话各自开分支（`docs/team-readme`、`docs/agents-md`、
`docs/pi-agent-borrowing`、`agent/chart-mvp-sync`），合并后不删，三天内从 2 条涨到 6 条，
...
> ⚠️ 本节**仅在用户明确要求新建分支时**适用。默认路径见 §4.1。

规则：

| MUST | MUST NOT |
| --- | --- |
| 必须先获得用户同意才能创建 | 未经同意创建，哪怕只是「先放着」 |
| 进 main 只能：从 main 开干净分支 → cherry-pick/选文件 → PR | 整支 merge/squash 隔离分支进 main |
| 合入后立即删除并打归档标签 | 合完留着「以后可能有用」 |
| 同样适用 §2.2 文件黑名单 | 以「未验证」为由提交 sqlite/密钥 |

远端活跃分支总数目标 **≤ 3**（`main` + 专属工作分支 + 最多 1 条临时分支）。
...
**例外（不要求二次确认）**：添加协作者 —— 用户已给出 GitHub 用户名即可直接执行（见 §5.2 / §8.3）。
...
**规则（按 2026-09-11 用户选定：拉人也不要确认）**：

- 用户**已给出** GitHub 用户名 → Agent **MUST 立即执行**邀请，**MUST NOT** 再问「确定要加吗」
- 用户**未给出**用户名 → 才询问用户名；**MUST NOT** 猜测用户名或用「最近联系人」
- 权限默认 `push`（Write）；用户未另行指定时不要再问权限
- 邀请是可撤销的协作邀请，不是改 `main`/删库；按上表执行即可
...
1. 若句中**已带 GitHub 用户名** → **不要再问**，直接执行 §5.2
2. 若只说「拉人 / 拉队友」但**没有用户名** → 问「GitHub 用户名是什么」
3. 权限默认 `push`，不另行确认
4. 执行后复述：已发邀请、对方需 Accept、如何 clone
5. 仍禁止猜测用户名
