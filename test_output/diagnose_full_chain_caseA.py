...
"""用例A（无毛利率，避开派生计算拦截）：驱动 Agent1-5 真实全链路。"""
import asyncio
import json
import os
import sys
...
async def main():
    ...
    state = create_pipeline_state(
        project_id="project-real-caseA",
        run_id="run-real-caseA",
        input_data=base_input(),
        review_stages=[StageName.DATA_FETCH, StageName.DATA_INTERPRET],
    )
    config = {"configurable": {"thread_id": "run-real-caseA"}}
    ...
    for i in range(10):
        ...
        if info.get("stage") == "data_interpret":
            ...
            dump(di_data.get("collaboration_requests"), 2000)
    ...
    print("data_interpret keys:", list(di_data.keys()))

    ch = stage_results.get("chart_design", {}) or {}
    ch_data = ch.get("data", {}) or {}
    print("chart_design keys:", list(ch_data.keys()))
    charts = ch_data.get("charts") or ch_data.get("chart_specs") or []
    print("chart count:", len(charts))

    cw = stage_results.get("chapter_writing", {}) or {}
    cw_data = cw.get("data", {}) or {}
    print("chapter_writing keys:", list(cw_data.keys()))

    fusion = stage_results.get("report_fusion", {}) or {}
    print("fusion status:", fusion.get("status"), "error:", fusion.get("error"))
    artifacts = fusion.get("artifacts") or []
    print("fusion artifacts:", json.dumps(artifacts, ensure_ascii=False, default=str)[:2000])
    print("DONE caseA run")
