...
"""用例A v4：显式传 RuntimePolicy(stage_timeout=600) 驱动A1-A5，落盘stage_results。"""
import asyncio
import json
import sys
...
async def main():
    ...
    policy = RuntimePolicy(stage_timeout_seconds=600, workflow_timeout_seconds=1800)
    graph = build_pipeline_graph(registry, checkpointer=InMemorySaver(), runtime_policy=policy)
    state = create_pipeline_state(
        project_id="project-real-caseA4",
        run_id="run-real-caseA4",
        input_data=base_input(),
        review_stages=[StageName.DATA_FETCH, StageName.DATA_INTERPRET],
    )
    config = {"configurable": {"thread_id": "run-real-caseA4"}}
    ...
    with open("/Users/Zhuanz1/PycharmProjects/同花顺/test_output/caseA4_stage_results.json", "w", encoding="utf-8") as f:
    ...
    for ch in charts[:20]:
        print("  chart:", json.dumps(ch, ensure_ascii=False, default=str)[:500])
    ...
    fusion = out.get("report_fusion", {}) or {}
    print("fusion artifacts:", json.dumps(fusion.get("artifacts"), ensure_ascii=False, default=str)[:1500])
