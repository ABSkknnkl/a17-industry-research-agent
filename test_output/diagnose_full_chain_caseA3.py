...
"""用例A v3：放宽stage超时(BUG-B已记录) 驱动A1-A5，落盘stage_results。"""
import asyncio
import json
import sys
...
async def main():
    settings.STAGE_TIMEOUT_SECONDS = 600
    ...
    state = create_pipeline_state(
        project_id="project-real-caseA3",
        run_id="run-real-caseA3",
        input_data=base_input(),
        review_stages=[StageName.DATA_FETCH, StageName.DATA_INTERPRET],
    )
    config = {"configurable": {"thread_id": "run-real-caseA3"}}
    ...
    with open("/Users/Zhuanz1/PycharmProjects/同花顺/test_output/caseA3_stage_results.json", "w", encoding="utf-8") as f:
