...
def test_real_agent_readiness_reports_codes_without_secret_values() -> None:
    configured = _production_settings(
        REAL_AGENTS_ENABLED=True,
        LLM_API_KEY=SecretStr("do-not-leak-llm-secret"),
        IWENCAI_API_KEY=None,
        SKILLHUB_API_KEY=None,
    )

    issues = runtime_configuration_issues(configured)
