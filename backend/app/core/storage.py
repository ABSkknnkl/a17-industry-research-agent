...
class StorageManager:
    # run_id 只允许字母/数字/点/下划线/连字符；文件名同理但更严（不得含路径分隔符）。
    ...
    def _safe_run_dir(self, run_id: str) -> Path:
        ...
        为什么需要：`base_dir / run_id` 在 run_id 含 `..` 或为绝对路径时会逃逸到工作区外，
        导致越界创建目录、甚至 `delete_run` 越界删除（缺陷 D-01）。这里做白名单 + 归属双重校验。

        Args:
            run_id: 运行标识，形如 `run-20260923094843-353`。

        Returns:
            base_dir 下的目标目录（Path，尚未创建）。

        Raises:
            ValueError: run_id 为空、含非法字符、为 `.`/`..`，或解析后不在 base_dir 之内。
        """
        ...
        if target == base or not target.is_relative_to(base):
            raise ValueError(f"run_id 越界（不得指向 base_dir 之外）: {run_id!r}")
        return target
    ...
    def _safe_file_name(cls, file_name: str) -> str:
        ...
        Raises:
            ValueError: 文件名为空、含路径分隔符或非法字符。
        """
        if not isinstance(file_name, str) or not cls._FILE_NAME_RE.match(file_name):
            raise ValueError(f"非法文件名: {file_name!r}")
        return file_name
    ...
    def save_artifact_file(self, run_id: str, filename: str, content: bytes | str) -> Path:
        ...
        target = run_dir / "artifacts" / self._safe_file_name(filename)
