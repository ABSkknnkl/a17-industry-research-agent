...
class StorageManager:
    ...
    def get_run_dir(self, run_id: str, *, create: bool = True) -> Path:
    ...
    def _load_title(self, run_dir: Path, data: dict) -> str:
        """与同花顺 RunSummary.title 对齐：industry_topic > project_id > run_id。"""
        topic = ""
        req_input = data.get("input_data") or {}
        if isinstance(req_input, dict):
            topic = str(req_input.get("industry_topic") or "").strip()
        if not topic:
            ...
            if input_file.exists():
                try:
                    ...
                    if isinstance(raw, dict):
                        topic = str(raw.get("industry_topic") or "").strip()
                    elif isinstance(payload, dict):
                        topic = str(payload.get("industry_topic") or "").strip()
                except Exception:
                    topic = ""
        if topic:
            return topic[:100]
        project_id = str(data.get("project_id") or "").strip()
        return (project_id or str(data.get("run_id") or run_dir.name))[:100]
    ...
    def load_state(self, run_id: str) -> WorkflowState | None:
        state_file = self.get_run_dir(run_id, create=False) / "state.json"
    ...
    def load_revision(self, run_id: str, revision: int) -> WorkflowState | None:
        rev_file = self.get_run_dir(run_id, create=False) / "revisions" / f"{revision}.json"
    ...
    def list_revisions(self, run_id: str) -> RevisionListResponse:
        ...
        rev_dir = self.get_run_dir(run_id, create=False) / "revisions"
    ...
    def list_runs(self, offset: int = 0, limit: int = 20) -> RunListResponse:
        ...
        for run_dir in self.base_dir.iterdir():
            ...
            try:
                ...
                title = self._load_title(run_dir, d)
