...
from backend.app.engine.state_machine import RevisionConflictError, engine
...
async def submit_review_endpoint(run_id: str, req: ReviewRequest) -> WorkflowState:
    ...
    try:
        ...
        logger.exception(f"审核提交失败: {run_id} - {e}")
...
async def delete_run_endpoint(run_id: str):
    """删除历史任务及其所有本地产物"""
    run_dir = storage.get_run_dir(run_id, create=False)
    if not run_dir.exists() or not (run_dir / "state.json").exists():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
