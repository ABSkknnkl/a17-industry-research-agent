...
- Markdown、单文件HTML与PDF共用同一套中文证据展示目录：正文使用“来源1/来源2”式可读引用，文末生成“来源与证据索引”，正式产物不展示`E/C/DQ/FC/CH/SEC`等机器编号。
- 内部机器编号仍保留在Agent 2—5的结构化契约、产物清单和运行日志中，用于去重、质量校验和数据血缘；展示层中文化不会削弱审计能力。
- 同一材料支持多个指标时合并为一条来源记录；旧版结果缺少来源元数据时显示“来源信息待补充”，不会把机器ID降级暴露给用户，也不会编造来源。
