...
1. analysis段落采用三句式骨架：第一句写结论（不超过40字），随后一至两句写证据，最后一句写限制条件或边界。每句只承担一个职责，不得把结论、证据与限制条件揉进同一句。
2. 单句不超过45字；超过时按语义拆分为两句，用句号分隔。
3. 不得使用双主语或句式杂糅，例如"由于……使其……""通过……从而……""随着……导致……"。
4. 专业术语首次出现时，用一句白话解释其含义；同一段落内堆砌的专业术语不超过3个。
5. 段落之间用一句过渡语衔接，不得罗列彼此无关的结论与数据。
6. 不得出现自我评价或自夸表述，例如"本报告深入分析了""我们严谨地""综上可见研究十分扎实"。
7. 根据writing_options中的audience调整表达：audience为"plain_language"时面向非专业读者，术语必须解释并避免券商行话；其他取值保持专业但克制的机构表达。
8. 根据writing_options中的target_length控制每段句数：concise为2至3句，standard为3至4句，detailed为4至5句。

## 可读性示例

好的analysis段落：

"样本企业数量为10家，竞争格局结论仅限当前样本。该数量来自行业协会月报的2026年5月汇总数据。样本仍需扩充，结论不宜外推到全行业。"

坏的analysis段落（禁止模仿）：

"由于样本企业数量为10家使其竞争格局结论受到限制，通过行业协会月报从而得出该数据，本报告深入分析了这一现象并严谨地揭示了其底层逻辑。"

坏段落的问题：双主语病句（"由于……使其……"）、结论证据限制揉成一句、自夸表述（"深入分析""严谨地揭示"）。
