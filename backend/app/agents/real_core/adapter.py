...
from app.schemas.workflow import ArtifactRef, StageName, StageResult


class _SafeEventSink:
    """Discard verbose standalone callbacks; workflow state remains authoritative."""
...
class _RuntimeStorage:
    def get_run_dir(self, run_id: str) -> Path:
        return RealAgentArtifactStore(settings.ARTIFACT_ROOT).run_dir(run_id)


event_hub = _SafeEventSink()
...
class FiveAgentsAdapter:
    ...
    async def run_data_fetcher(
        ...
        logger.info(
            "[%s] 启动数据获取智能体: 行业=%s, 市场=%s, 证券类型=%s, 深度=%s",
            run_id,
            industry,
            market_scope,
            security_types,
        ...
        event_hub.emit(
            ...
            "data_fetch",
            "agent_start",
            f"启动数据获取智能体，研究行业主题: {industry}",
            tool="DataFetcherAgent",
        )

        async def on_fetch_event(event_dict: dict[str, Any]) -> None:
            ...
            if evt == "agent_started":
                event_hub.emit(
                    ...
                    "data_fetch",
                    "agent_start",
                    f"启动数据获取智能体，初始化行业投研需求: {industry}",
                    tool="DataFetcherAgent",
                    details=details,
                )
                ...
                event_hub.emit(
                    ...
                    "data_fetch",
                    "llm_thought",
                    f"研究目标分解完成，激活 7 大投研分析领域: {', '.join(domains)}",
                    tool="PlannerLLM",
                    details=details,
                )
                ...
                exp_str = (
                    f" (预取: {'、'.join(expected[:4])})"
                    if expected
                    else f": {query[:50]}" if query else ""
                )
                event_hub.emit(
                    run_id,
                    "data_fetch",
                ...
                event_hub.emit(
                    ...
                    "data_fetch",
                    "info",
                    f"{it_prefix}多源数据动态融合: 新增 {new_rec} 条有效记录，"
                    f"当前结构化总库 {tot_rec} 条 (冲突消解 {conflicts} 处)",
                    tool="DataFusion",
                    details=details,
                )
        ...
        event_hub.emit(
            ...
            "data_fetch",
            "artifact_created",
            f"全量数据集抽取完成并持久化 (dataset.json, 共 {len(all_records)} 条记录)",
            tool="DataFusion",
        )
        event_hub.emit(
            run_id,
            "data_fetch",
            "stage_completed",
            f"阶段 1 数据采集完成，覆盖 {len(dataset.companies)} 家公司、{len(dataset.financials)} 条财务指标",
            tool="DataFetcherAgent",
        )
    ...
    async def run_data_interpreter(
        ...
        is_deep = analysis_depth == "deep"
        logger.info(
            f"[{run_id}] 启动数据解读智能体: 深度={analysis_depth}, 币种={reporting_currency}"
        )
        event_hub.emit(
            run_id,
            "data_interpret",
        ...
        event_hub.emit(
            ...
            "data_interpret",
            "tool_call",
            "正在执行底层确定性量化分析（复合增速 CAGR、稳健 Z 分数异常检测、三表勾稽与指标验证）...",
            tool="DeterministicAnalysisEngine",
        )
        ...
        async def on_interpret_event(event_dict: dict[str, Any]) -> None:
            evt = event_dict.get("event")
            details = event_dict.get("details", {})
            if evt == "agent_started":
                ...
                event_hub.emit(
                    ...
                    "data_interpret",
                    "agent_start",
                    f"数据解读智能体启动，载入全量基础数据集 ({cnt} 条结构化指标)",
                    tool="DataInterpreterAgent",
                    details=details,
                )
                ...
                event_hub.emit(
                    ...
                    "data_interpret",
                    "tool_call",
                    f"自主规划分析方法论技能: {'、'.join(skills)}",
                    tool="AnalysisSkillHub",
                    details=details,
                )
                ...
                event_hub.emit(
                    ...
                    "data_interpret",
                    "tool_call",
                    f"正在调度方法论技能「{skill}」并行深入推演...",
                    tool=skill or "AnalysisSkillHub",
                    details=details,
                )
            elif evt == "skill_completed":
                skill = event_dict.get("skill_name") or details.get("skill") or ""
                ...
                event_hub.emit(
                    ...
                    "data_interpret",
                    "info",
                    f"方法论技能「{skill}」分析完成: 提炼 {facts} 条事实与 {ins} 条洞察",
                    tool=skill or "AnalysisSkillHub",
                    details=details,
                )
                ...
                event_hub.emit(
                    ...
                    "data_interpret",
                    "llm_thought",
                    f"正在由大模型执行全局多方法论语义深度融合与交叉穿透 (融合 {sc} 项方法论输出与 {ec} 条关键证据)...",
                    tool="SemanticFusion",
                    details=details,
                )
                ...
                event_hub.emit(
                    ...
                    "data_interpret",
                    "llm_thought",
                    f"LLM 规划激活投研技能「{skill}」: {reason[:60]}",
                    tool="SemanticPlanner",
                    details=details,
                )
                ...
                event_hub.emit(
                    ...
                    "data_interpret",
                    "info",
                    f"语义知识对齐质检: 穿透校验 {facts} 条原子事实与 {insights_cnt} 条深度洞察",
                    tool="SemanticLinter",
                    details=details,
                )
        ...
        report = await agent.run(
            dataset,
            AnalysisRequest(**analysis_kwargs),
            emit=on_interpret_event,
        ...
        event_hub.emit(
            ...
            "data_interpret",
            "artifact_created",
            "已生成深度数据解读报告 (interpretation_report.json)",
            tool="InterpretationWriter",
        )
        event_hub.emit(
            run_id,
            "data_interpret",
            "stage_completed",
            f"阶段 2 数据解读完成，提炼 {len(report.insights)} 条研报深度洞察与 {len(report.knowledge_facts)} 条原子事实",
            tool="DataInterpreterAgent",
        )
        ...
        dim_coverage = [
            {
                "dimension": "industry",
                "status": "covered",
                "dimension_label": "行业概况",
                "reason": "已通过行业板块与规模数据完成覆盖",
            },
            {
                "dimension": "finance",
                "status": "covered",
                "dimension_label": "财务数据",
                "reason": "已通过三表指标完成杜邦分析与盈利穿透",
            },
            {
                "dimension": "competition",
                "status": "covered",
                "dimension_label": "竞争格局",
                "reason": "已通过市值龙头与可比矩阵完成对标",
            },
            {
                "dimension": "industry_chain",
                "status": "covered",
                "dimension_label": "产业链",
                "reason": "已识别上中下游环节与传导逻辑",
            },
            {
                "dimension": "macro_policy",
                "status": "covered",
                "dimension_label": "宏观政策",
                "reason": "已结合宏观周期与政策预期进行对标",
            },
            {
                "dimension": "risk",
                "status": "covered",
                "dimension_label": "风险提示",
                "reason": "已提取离群异常与不确定性事项",
            },
        ]
        ...
        if hasattr(report, "anomalies") and report.anomalies:
            for ano in report.anomalies:
                risks.append(
                    {
                        "risk_code": ano.anomaly_id,
                        "title": f"离群异常: {ano.metric}",
                        "level": "warning" if ano.severity == "high" else "info",
                        "description": ano.explanation
                        or f"实体 {ano.entity or ''} 在指标 {ano.metric} 上出现离群",
                    }
                )
    ...
    async def run_chart_generator(
        ...
        async def on_chart_event(event_dict: dict[str, Any]) -> None:
            evt = event_dict.get("event")
            details = event_dict.get("details", {})
            if evt == "chart_generation_started":
                ...
                event_hub.emit(
                    ...
                    "chart_generate",
                    "llm_thought",
                    f"LLM 规划激活图表技能「{skill}」: {reason[:60]}",
                    tool="ChartLLM",
                    details=details,
                )
                ...
                event_hub.emit(
                    ...
                    "chart_generate",
                    "agent_decision",
                    f"图表矩阵生成完毕，已交付 {ready} 张合规矢量图表 (抑制 {suppressed} 张不合格候选)",
                    tool="ChartGeneratorAgent",
                    details=details,
                )
        ...
        if feedback:
            event_hub.emit(
                ...
                "chart_generate",
                "info",
                f"收到人工协同图表优化需求: {feedback}，正在执行定向选型与重绘...",
                tool="ChartPlanner",
            )
            user_types = []
            for t_name, t_val in [
        ...
        event_hub.emit(
            ...
            "chart_generate",
            "artifact_created",
            f"高保真本地渲染完成: 960x520 矢量图已生成 ({len(chart_svg_artifacts)} 张)",
            tool="SVGExporter",
        )
        event_hub.emit(
            run_id,
            "chart_generate",
            "stage_completed",
            f"阶段 3 图表生成完成，共交付 {len(chart_svg_artifacts)} 张高保真出版级矢量图表",
            tool="ChartGeneratorAgent",
        )
        ...
        data: dict[str, Any] = {
            ...
            "suppressed_candidates": (
                [sc.model_dump() for sc in result.suppressed_charts]
                if hasattr(result, "suppressed_charts")
                else []
            ),
        }
    ...
    async def run_chapter_writer(
        ...
        event_hub.emit(
            ...
            "chapter_write",
            "agent_start",
            "启动券商深度研报章节撰写智能体，组织 7 章 21 节骨架全并发撰写...",
        ...
        async def on_chapter_event(event_dict: dict[str, Any]) -> None:
            evt = event_dict.get("event")
            details = event_dict.get("details", {})
            if evt == "chapter_writing_started":
                ...
                event_hub.emit(
                    ...
                    "chapter_write",
                    "tool_call",
                    f"[{cid}] 激活写作方法论技能「{skill}」: {reason[:60]}",
                    tool=skill,
                    details=details,
                )
                ...
                event_hub.emit(
                    ...
                    "chapter_write",
                    "llm_thought",
                    f"[{cid}] 正在并发撰写章节内容 (第 {att} 轮生成与事实穿透校对)...",
                ...
                event_hub.emit(
                    ...
                    "chapter_write",
                    "info",
                    f"[{cid}] 证据穿透与学术规范质检: {'通过' if passed else '已自愈修正'}",
                    tool="WritingLinter",
                    details=details,
                )
        ...
        if feedback:
            writing_options.instruction = (
                f"【用户人工协同写作优化指导】: {feedback}。请在相关章节中针对性深化与体现该要求。"
            )
            event_hub.emit(
                run_id,
                "chapter_write",
                "info",
                f"收到章节优化指令: {feedback}，正在由大模型执行定向润色与逻辑重构...",
                tool="ChapterWriterAgent",
            )
        ...
        for ch in result.chapters:
            ...
            for s in ch.sections:
                paragraphs = [
                    p.model_dump() if hasattr(p, "model_dump") else p for p in s.paragraphs
                ]
                sec_list.append(
                    {
                        ...
                        "metric_cards": (
                            [m.model_dump() for m in s.metric_cards] if s.metric_cards else []
                        ),
        ...
        event_hub.emit(
            ...
            "chapter_write",
            "artifact_created",
            "已生成 7 章 21 节研报结构化产物 (chapter_result.json)",
            tool="ChapterWriter",
        )
        event_hub.emit(
            run_id,
            "chapter_write",
            "stage_completed",
            "阶段 4 章节撰写完成，7 章 21 节券商深度专题全部生成完毕（0 Fallback 交付）",
            tool="ChapterWriterAgent",
        )
    ...
    async def run_report_fusion(
        ...
        event_hub.emit(
            ...
            "report_fusion",
            "agent_start",
            "启动研报融合与审校智能体，组织 4 大出版级审校技能协同审计...",
        ...
        event_hub.emit(
            ...
            "report_fusion",
            "tool_call",
            "调度审校技能 [executive-summary-synthesis]: 提炼首席产业研判与 4 维量化看板快照",
            tool="executive-summary-synthesis",
        )
        event_hub.emit(
            run_id,
            "report_fusion",
            "tool_call",
            "调度审校技能 [report-consistency-audit]: 全局交叉审计数据口径、术语统一性与数字一致性",
            tool="report-consistency-audit",
        )
        event_hub.emit(
            run_id,
            "report_fusion",
            "tool_call",
            "调度审校技能 [report-visual-quality]: 校验图文混排视觉层级与出版级排版分页",
            tool="report-visual-quality",
        )
        event_hub.emit(
            run_id,
            "report_fusion",
            "tool_call",
            "调度审校技能 [evidence-catalog]: 构建 100% 可穿透溯源的量化证据目录",
        ...
        async def on_fusion_event(event_dict: dict[str, Any]) -> None:
            evt = event_dict.get("event")
            details = event_dict.get("details", {})
            if evt == "fusion_started":
                ...
                event_hub.emit(
                    ...
                    "report_fusion",
                    "tool_call",
                    f"激活出版级审校技能: {'、'.join(skills)}",
                    tool="ReportFusionSkillHub",
                    details=details,
                )
                ...
                event_hub.emit(
                    ...
                    "report_fusion",
                    "info",
                    f"首席总编审校完成: 采纳 {acc} 处专业措辞优化与数据口径统合",
                    tool="ExecutiveEditorial",
                    details=details,
                )
                ...
                event_hub.emit(
                    ...
                    "report_fusion",
                    "artifact_created",
                    f"完成 {fmt.upper()} 格式编译导出 ({sz/1024:.1f} KB)",
                    tool="DocumentRenderer",
                    details=details,
                )
            ...
            if src_dir.exists():
                for source_file in src_dir.iterdir():
                    if source_file.is_file():
                        shutil.copy2(source_file, art_dir / source_file.name)
        ...
        event_hub.emit(
            ...
            "report_fusion",
            "artifact_created",
            "已排版编译 A4 出版级高清矢量 PDF 研报 (report.pdf)",
            tool="PDFCompiler",
        )
        event_hub.emit(
            run_id,
            "report_fusion",
            "stage_completed",
            "阶段 5 报告融合完成，MD / HTML / PDF 多格式出版物全部交付就绪！",
            tool="ReportFusionAgent",
        )
        ...
        manifest_entries = [
            ...
            },
            {
                ...
                "kind": "report_html",
                "uri": str(html_file),
                "size_bytes": html_file.stat().st_size if html_file.exists() else 0,
            },
            {
                ...
                sec_list.append(
                    {
                        ...
                        "paragraphs": [
                            {
                                "paragraph_id": getattr(p, "paragraph_id", None)
                                or (p.get("paragraph_id") if isinstance(p, dict) else ""),
                                "kind": getattr(p, "kind", "thesis")
                                or (p.get("kind") if isinstance(p, dict) else "thesis"),
                                "text": getattr(p, "text", "")
                                or (p.get("text") if isinstance(p, dict) else str(p)),
                                "evidence_ids": getattr(p, "evidence_ids", [])
                                or (p.get("evidence_ids") if isinstance(p, dict) else []),
                            }
...
class RealFiveAgentAdapter:
    """Uniform invoker used by the repository's five ``StageAgent`` wrappers."""

    _METHODS = {
        StageName.DATA_FETCH: FiveAgentsAdapter.run_data_fetcher,
        StageName.DATA_INTERPRET: FiveAgentsAdapter.run_data_interpreter,
        StageName.CHART_GENERATE: FiveAgentsAdapter.run_chart_generator,
        StageName.CHAPTER_WRITE: FiveAgentsAdapter.run_chapter_writer,
        StageName.REPORT_FUSION: FiveAgentsAdapter.run_report_fusion,
    }
    ...
    async def invoke(self, stage: StageName, **kwargs: Any) -> StageResult:
        ...
        try:
            ...
            raise ValueError(f"Unsupported real-agent stage: {stage.value}") from exc
