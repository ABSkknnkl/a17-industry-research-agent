...
class RevisionConflictError(Exception):
    """expected_revision 与当前 revision 不一致（前端应刷新后重试）。"""
...
class WorkflowEngine:
    ...
    async def handle_review(self, req: ReviewRequest) -> WorkflowState:
        ...
        # 版本冲突门禁：与同花顺一致，expected_revision 不匹配时拒绝并提示前端刷新
        if req.expected_revision != state.revision:
            raise RevisionConflictError(
                f"Revision conflict: expected r{req.expected_revision}, current r{state.revision}"
            )
        ...
        input_file = run_dir / "input_data.json"
        review_stages = ["data_fetch", "data_interpret"]
        ...
        if req.action in ("approve", "accept_recommendation", "accept_with_risks"):
            ...
            state.stage_results[current_stage].status = "approved"
    ...
    async def _execute_stage(
        ...
        try:
            ...
            # 判断下一步流转策略：
            # 若该阶段在 review_stages 中，且不是终态，则进入 waiting_review 等待人工审核！
            if stage in review_stages and stage != "report_fusion":
